        ...
        <div className="style-categories">
          ...
          <div className="style-grid">
            {mockStyles.filter(s => s.category === 'free').map(style => (
              <div key={style.id} className={`style-card ${selectedStyle === style.id ? 'active' : ''}`} onClick={() => setSelectedStyle(style.id)} style={{ borderColor: selectedStyle === style.id ? style.color : 'transparent' }}>
                <div className="style-icon">{style.icon}</div><h3>{style.name}</h3><p>{style.description}</p><div className="style-music">{style.music}</div><div className="style-tag free">免费</div>
              </div>
            ))}
          </div>
          <h3>付费模板</h3>
          <div className="style-grid">
            {mockStyles.filter(s => s.category === 'paid').map(style => (
              <div key={style.id} className={`style-card paid`} onClick={() => alert('此模板需要购买')} style={{ borderColor: 'transparent' }}>
                <div className="style-icon">{style.icon}</div><h3>{style.name}</h3><p>{style.description}</p><div className="style-music">{style.music}</div><div className="style-tag paid-tag">付费</div>
